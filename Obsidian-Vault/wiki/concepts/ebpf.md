...
- low-overhead
- no-instrumentation
summary: Extended Berkeley Packet Filter — a Linux kernel technology that enables safe, low-overhead program execution in the kernel for observability, networking, and profiling without application instrume...
sources:
...
[[concepts/ebpf]] is a Linux kernel technology that enables safe, sandboxed programs to run in the kernel at runtime without modifying kernel source code or loading kernel modules. It has become a foundational technology for observability, networking, and profiling.

## Role in Profiling

[[concepts/ebpf]] is the mechanism that enables [[concepts/continuous-profiling|continuous profiling]] with minimal overhead. Because profiling is performed at the kernel level:
...
The Linux kernel, via [[concepts/ebpf|eBPF]], can:
...
[[concepts/ebpf]]-based profiling tools can be deployed as:
...
Profiling itself dates back to the 1970s with IBM researchers. [[concepts/ebpf]] emerged much later (circa 2014) and has revolutionized what's possible with kernel-level observability in Linux.
