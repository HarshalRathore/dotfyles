...
## Current Ingest
- [[references/how-a-gpu-actually-works]] — Akshay Pachaar's GPU fundamentals X Article: arithmetic intensity, roofline break-even (H100 ~295 ops/byte), decode memory-bound floor (~24 tok/s for 70B fp16).
- [[references/vibecoding-gpu-kernels]] — maharshi's X Article: LLM-written GPU kernels in a verifiable loop (compile → correctness → benchmark → roofline), rung harnesses, NCU profiling.
- [[concepts/arithmetic-intensity]] — NEW concept: work per byte; the ratio that decides compute- vs memory-bound against a chip's break-even point.
- [[concepts/gpu-memory-hierarchy]] — NEW concept: register file → shared/L1 → L2 → HBM ladder; bandwidth, not latency, is the hard ceiling.
- [[concepts/agent-driven-kernel-development]] — NEW concept: kernel writing as an embarrassingly verifiable agent task.
- [[misc/web-x-com-i-status-2087928032904523980]] — Pachaar's post page (1,234 bookmarks at capture).
- [[misc/web-x-com-i-status-2086442755748970889]] — maharshi's post page (831 bookmarks at capture).
- [[entities/maharshi]] — ML perf optimizer at FAL; entity created.
- [[entities/dailydoseofds]] — Daily Dose of Data Science entity created.
- [[entities/lightning-ai]] — Lightning AI stub created.
- [[entities/akshay-pachaar]] — updated with the GPU article + DDDS co-founder facts.


## Recently Created
- [[misc/web-x-com-i-status-2087928032904523980]] — How a GPU Actually Works — Akshay Pachaar's X Article post page (1,234 bookmarks at capture)
- [[references/how-a-gpu-actually-works]] — Pachaar X Article deep-dive: memory ladder, warp divergence, latency hiding, break-even ratio (H100 295 ops/byte), decode floor
- [[concepts/arithmetic-intensity]] — NEW concept: work per byte; break-even/ridge point; roofline model
- [[concepts/gpu-memory-hierarchy]] — NEW concept: register file → shared/L1 → L2 → HBM; bandwidth the hard ceiling
- [[entities/dailydoseofds]] — Daily Dose of Data Science entity
- [[entities/lightning-ai]] — Lightning AI stub (Pachaar's ex-employer)
- [[misc/web-x-com-i-status-2086442755748970889]] — Vibecoding GPU Kernels — maharshi's X Article post page (831 bookmarks at capture)
- [[references/vibecoding-gpu-kernels]] — maharshi X Article deep-dive: verifiable kernel loop, DSL context dirs, rung harnesses, NCU profiling
- [[concepts/agent-driven-kernel-development]] — NEW concept: embarrassingly verifiable kernel writing via LLM agents
- [[entities/maharshi]] — Maharshi entity (ML perf optimizer at FAL)
...
- [[skills/reddit-startup-marketing]] — How to get your first 100 users from reddit without getting banned — A founder's guide to Reddit marketing: account warming, strategic posting/commenting, ban avoidance, and attribution tracking
...
## Recently Updated
- [[concepts/ai-harness]] — Kernel-dev verifiable loop merged as concrete harness instance (maharshi, 2026-08-09)
- [[concepts/gpu-profiling]] — NCU/PTX/SASS dumps as agent feedback merged (maharshi, 2026-08-09)
- [[entities/akshay-pachaar]] — How a GPU Actually Works section added; DDDS co-founder facts (2026-08-13 article)
- [[concepts/compute-bound-vs-memory-bound]] — Arithmetic-intensity framing, 24 tok/s decode floor, ~300-sequence break-even merged (Pachaar, 2026-08-13)
- [[concepts/gpu-utilization]] — 'Scheduled vs useful work' monitoring caveat merged (Pachaar, 2026-08-13)
- [[concepts/continuous-batching]] — Stub enriched with batching economics (Pachaar, 2026-08-13)
...
- 2026-08-16 — Orchestrated dual X-Article ingest (2 batches, 7 parallel sub-agents): Akshay Pachaar's "How a GPU Actually Works" (GPU fundamentals: arithmetic intensity, roofline break-even ~295 ops/byte on H100, decode memory-bound floor ~24 tok/s for 70B fp16) and maharshi's "vibecoding gpu kernels" (LLM-written kernels in a verifiable compile→correctness→roofline loop; DSL context directories, rung harnesses, NCU profiling). 10 pages created (2 misc, 2 references, 3 concepts, 3 entities), 6 existing pages updated (akshay-pachaar, compute-bound-vs-memory-bound, gpu-utilization, continuous-batching, gpu-profiling, ai-harness). The two articles cross-link: one supplies the roofline theory, the other the agentic workflow that targets it.

- 2026-08-15 — Three authenticated X Article ingests: PostHog's agent-skill engineering guide, Ashpreet Bedi's recursive auto-improvement loop, and camelAI's Durable Object agent architecture. Created 11 durable/reference/entity pages and updated the existing context, eval, observability, loop, harness, Cloudflare, Pi, and PostHog threads.

- 2026-08-15 — Cordis paper ingest (github.com/cordiverse/paper). 'A Programming Paradigm for Spatiotemporal Composability' (Shi, Zhang & Cui, draft 2026-08-13, 88pp) gives dynamic composition its first full formal foundation: revertible effects (every context transformation carries an inverse the runtime tracks — local temporal composability) and reactive coeffects (context changes notify components as activating/deactivating/neutral — local spatial composability), unified in a recursive context type Γ∞ (the context paradigm), plus a calculus of dynamic composition with metatheory (preservation, recovery exactness, provider-withdrawal ordering, progress, confluence). Implemented as Cordis (meta-framework: ctx.effect/ctx.set/ctx.use, declarative loader with config reconciliation + HMR), validated by Koishi (4000+ plugins). 16 pages created (deep-dive + repo landing + 7 concepts + 7 entities); 2 existing pages updated (concepts/ai-harness — dynamic composition as harness infrastructure; entities/deepseek — DeepSeek-AI affiliation via Tianyi Cui/Yifan Shi). New vault thread: dynamic composition as the lifecycle layer beneath the five-subsystem harness model, with self-evolving agent harnesses as the motivating application and future validation.
...
## Recent Activity
- [2026-08-18T17:12:00Z] Ingested GTM agent machine guide by @EXM7777 (Machina) — 9 pages covering automated GTM workflows for agencies, SaaS, and infoproducts using AI agents, including the Viktor shortcut, knowledge base patterns, four-part workflow template, and SEO/answer engine layer.

## Active Threads
- **GPU fundamentals → agent kernel harnesses**: roofline/arithmetic-intensity (Pachaar) + verifiable kernel loops (maharshi) — both authors promise follow-ups (profiler reading vs break-even, KV-cache growth, serving-engine batching; expert-kernel scalability of the loop).
- **Closet-first wardrobe AI (project: ai-wardrobe-india)**: The closet-first memory layer (catalog + taste profile + wear history) is unclaimed — PointAI/Google/Myntra are shop-first VTO and Kya Pehne is the only ethnic-first entrant, so the wedge is open but time-sensitive (~6–12 months). Product thesis: one-time structured wardrobe ingestion with retained originals/embeddings/confidence, occasion knowledge cards, outfit ranking-vs-generation, cheap-by-default model routing, and the moat claim that Indian ethnic-fashion preference data + a specialized ranker — not prompts/tools — is the durable defensibility.
...
- **Agent skill engineering**: PostHog's 226 internal skills and 187 `SKILL.md` files across 28 products reinforce progressive disclosure, source-linked regeneration, and selective skill creation over procedural instruction dumps.
- **Recursive auto-improvement**: Production agents can be improved by mining sessions into probes, reading live traces, changing one rule/tool/parameter, and rerunning failures toward a fixed point; held-out probes remain necessary to detect overfitting.
- **Edge-native agent harnesses**: camelAI separates a Durable Object-hosted brain from specialized Linux hands, storing state as SQLite/R2 data and replacing bash with credential-isolated Code Mode methods.
...
- **Evaluation**: Agent Arena, agent-as-judge, structured outputs, and telemetry-driven evaluation pipelines remain core infrastructure; session-mined probes now connect production traces directly to regression evals.
...
- **Dynamic composition for self-evolving harnesses (Cordis paper)**: revertible effects + reactive coeffects unified in a context type now give the vault a formal foundation for swapping harness components at runtime; the open question is whether the metatheory's pairwise-independence/confluence guarantees transfer to AI-generated harness modifications (the paper's stated future work).
## Key Takeaways
- Dynamic composition is the formal under-layer of harness engineering: revertible effects give teardown-derived-from-loading (no hand-written uninstall), reactive coeffects give dependency rewiring without ad hoc code, and the calculus's confluence result says the quiescent state is the one a static assembly would produce — the licensing result for reasoning about a Cordis app as statically assembled. Self-evolving agent harnesses (agents generating and deploying modifications to their own components) are named as the paradigm's motivating application and future validation direction. ^[inferred]
- The AI wardrobe wedge is occasion-first: Indian wedding/festival decisions (haldi/mehendi/sangeet/reception) are high-spend and high-anxiety, and the closet-first memory layer (catalog + taste + wear history) is unclaimed by shop-first VTO platforms — no scale player models saree draping or occasion semantics. ^[extracted]
- Unit economics support a free tier: AI cost is ~$0.03–2.80/user/mo (ingestion ~$0.001/item, styling ~$0.0014–0.0025/interaction, images $0.034–0.10), so bulk ingestion can be given away and image previews metered; affiliate (Flipkart 8%, Myntra 8–12%) is a complement, not a revenue model. ^[extracted]
- Model routing is a cost policy: cheap models handle routine/high-confidence cases, frontier multimodal models handle ambiguous/unknown/premium cases — benchmark blind and choose the cheapest passing threshold; wardrobe styling on gpt-5.6-luna is ≈$0.0014/interaction ≈ ₹0.12 (≈₹3.60 for 30), a time-sensitive estimate that must be recomputed before reuse. ^[extracted]
